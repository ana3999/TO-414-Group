...
#week_end_colorPlot(name = "Resort Hotel", minNumber = 70)
...
```{r}
#, fig.height = 5}
adultChild_colorPlot <- function(name, x = hotels, probabilityLimit = 9001, minNumber = 30){
  ...
  a <- aggregate(is_canceled ~ adults + children, data = x, mean, na.rm = TRUE)
  onesFrame <- aggregate(ones ~ adults + children, data = x, sum, na.rm = TRUE)
  ...
  colnames(filteredA) <- c("Adults", "Children", "Canceled")
  filteredA <- as.data.frame( filteredA[complete.cases(filteredA), ] )
  ...
  #[with(a, id %in% names(which(table(id)>=5))), ]
  
  qplot(filteredA$Adults,
        filteredA$Children,
        color = filteredA$Canceled,
        size = I(12),
        geom = "point",
    ...
    ggtitle(paste("Cancellation Probability as a Function of Number\n of Adults and Children for", name)) +
...
#adultChild_colorPlot(name = "Resort Hotel", minNumber = 5)
#adultChild_colorPlot(name = "Resort Hotel", probabilityLimit = 0.9)
```
...
# week_end_colorPlot(name = "Resort Hotel", minNumber = 0) this looks cool but isn't very useful
```
...
**cancellations by number of adults and children**
```{r}
#adultChild_colorPlot(name = "City Hotel")
```
